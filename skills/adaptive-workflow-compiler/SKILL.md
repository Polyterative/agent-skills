...
stages, artifacts, specialists, gates, and iteration budget required for a
high-quality result.

## Hard ordering rule

Compile the workflow before invoking any role skill. Do not edit production code
until the preparation phase is complete and its
...
- `references/workflow-examples.md` — load only if recipes alone leave
...
Record score, evidence, uncertainty, and override in the manifest. Effort is
...
- For product/UI behavior, produce 1-3 substantive stories and one primary
...
- Skip `OpportunityMap` unless the task is open-ended discovery, not a
  named area/story.
- For product/UI behavior, produce 5-12 substantive stories, a `StoryMap`,
...
- Produce an `InteractionContract` (merged `StateMatrix` +
  `InteractionSpecification`) with every applicable state as a testable
  assertion, plus an `AcceptanceTestPlan` covering every selected story and
  applicable `InteractionContract` row.
...
- Produce a complete `AcceptanceTestPlan` before implementation; every story,
  state, and recovery path has a named test, and risky slices may not
...
Produce a `WorkflowManifest` with:
...
The manifest is the execution contract. The coordinator may adapt it to new
...
Compose recipes when multiple intents apply. For example, a redesign with a
...
The compiler may not remove these stages to save effort. At MEDIUM and
...
- the work requires no judgment about what users need.
...
work: deciding what "better" means requires use cases and stories.
...
the `BehaviorContract` as input, and the manifest lists DISCOVER
...
- At LOW, the coordinator or single selected product/design role may produce the
  ...
  required only because the artifacts have distinct names.
...
- The quality role handles both strategy readiness and final QA in the same
  context; do not reload the skill between gates.
...
3. Record a compact value/cost estimate: expected user value, confidence, and
   the preparation-plus-execution cost implied by the classification.
...
6. Remove optional stages that add no required artifact or evidence, while
...
12. Record a compact manifest and `LoopRunRecord` in session state before role
    ...
    enforce the loop: a stage whose todo dependencies are not `done` must not
    start.
...
The compiler supports three iteration kinds (canonical definitions and
routing mechanics: `autonomous-development-loop` §Quality-driven full-pass
iteration):

### Local repair

Use when workflow and contracts remain correct but implementation or
